# GraphCast Post-Export Steps

This file starts after GraphCast outputs already exist on disk.

Use it when you already have one of these:

- `station_daily_tmax_tmin_graphcast.csv`
- a gridded GraphCast NetCDF/Zarr export with sub-daily `2m_temperature`
- a gridded GraphCast daily Tmax/Tmin export

## Goal

Validate GraphCast as an external baseline against the local station observations in `merged.csv`.
...
The evaluation script does not run GraphCast. It only evaluates exported predictions after they already exist.
...
- `csv_daily_tmax_tmin` if you already have `station_daily_tmax_tmin_graphcast.csv`
- `subdaily_t2m` if you have gridded sub-daily `2m_temperature`
- `daily_tmax_tmin` if you have gridded daily Tmax/Tmin

## Step 3. Run the evaluator

### Option A. GraphCast station daily CSV

```bash
python scripts/evaluate_gridded_baseline.py \
  --source-model graphcast \
  --prediction-path /path/to/station_daily_tmax_tmin_graphcast.csv \
  --mode csv_daily_tmax_tmin \
  --start-date 2024-01-01 \
  --end-date 2025-12-31
```

### Option B. GraphCast gridded sub-daily temperature

Use this if the export still contains the gridded `2m_temperature` field:
...
### Option C. GraphCast gridded daily Tmax/Tmin

```bash
python scripts/evaluate_gridded_baseline.py \
  --source-model graphcast \
  --prediction-path /path/to/graphcast_daily_predictions.nc \
...
- `outputs/external_baselines/graphcast/matched_station_predictions.csv`
- `outputs/external_baselines/graphcast/station_mae.csv`
- `outputs/external_baselines/graphcast/summary.json`
...
- if the evaluator runs cleanly and the summary numbers are plausible, freeze that GraphCast daily CSV as the baseline artifact for this iteration
...
- GraphCast export files
- `run_metadata_graphcast.json`
- evaluation outputs from `outputs/external_baselines/graphcast/`

That makes the result reproducible later.

## Optional next step

After the daily baseline is stable, extend the GraphCast export to the hourly precursor file:
